...
function kbTopics(treeType){return (KB?.topics||[]).filter(t=>t.treeType===treeType)}
function kbChildren(id){const root=kbTopicById(id);return (KB?.topics||[]).filter(t=>t.parentTopicId===id&&(!root?.treeType||t.treeType===root.treeType))}
...
function kbTopicOptions(selected='',treeType='information'){return kbTopics(treeType).map(t=>`<option value="${ke(t.id)}" ${t.id===selected?'selected':''}>${ke(t.name)}</option>`).join('')}
...
function kbTopicTree(){
  return (KB?.topicGroups||[]).map(group=>{
    const roots=kbTopics('information').filter(t=>t.groupId===group.id&&!t.parentTopicId);
...
function kbPostingTree(){
  return (KB?.topicGroups||[]).map(group=>{
    const roots=kbTopics('posting').filter(t=>t.groupId===group.id&&!t.parentTopicId);
    if(!roots.length)return '';
...
function kbKnowledgeRows(){
  ...
  return items.map(item=>{
    const source=kbSource(item.sourceId),topics=(item.topicIds||[]).map(kbTopicById).filter(t=>t?.treeType==='information');
...
function kbDialogs(){
  ...
  <dialog id="kbItemDialog" class="kb-dialog"><form class="kb-dialog-form" data-kb-item-form><div class="kb-dialog-head"><h2>Add knowledge</h2><button type="button" class="icon-action" data-kb-close="kbItemDialog" aria-label="Close">×</button></div><div class="kb-form-grid"><label class="kb-field"><span>Source</span><select name="sourceId">${KB.sources.map(s=>`<option value="${ke(s.id)}" ${s.id==='src-kb-manual'?'selected':''}>${ke(s.name)}</option>`).join('')}</select></label><label class="kb-field"><span>Type</span><select name="kind"><option value="idea">Idea</option><option value="fact">Fact</option><option value="guide">Guide</option><option value="community">Community signal</option><option value="comparison">Comparison</option><option value="watch">Watch</option></select></label><label class="kb-field wide"><span>Title</span><input name="title" required></label><label class="kb-field wide"><span>Summary / raw note</span><textarea name="summary" required></textarea></label><label class="kb-field"><span>Information topic</span><select name="topicId"><option value="">Unassigned</option>${kbTopicOptions('', 'information')}</select></label><label class="kb-field"><span>Source URL override</span><input name="sourceUrl" type="url" placeholder="Optional"></label></div><div class="kb-dialog-actions"><button type="button" class="secondary-btn" data-kb-close="kbItemDialog">Cancel</button><button class="primary-btn" type="submit">Save knowledge</button></div></form></dialog>
  <dialog id="kbContentDialog" class="kb-dialog"><form class="kb-dialog-form" data-kb-content-form><div class="kb-dialog-head"><h2>Create content idea</h2><button type="button" class="icon-action" data-kb-close="kbContentDialog" aria-label="Close">×</button></div><div class="kb-lineage" data-kb-lineage>Content will keep a traceable link to the selected knowledge source while using the editorial posting taxonomy.</div><input type="hidden" name="knowledgeItemId"><label class="kb-field"><span>Format</span><select name="type">${Object.entries(kbFormats).map(([v,l])=>`<option value="${v}">${l}</option>`).join('')}</select></label><label class="kb-field"><span>Posting topic</span><select name="topicId">${kbTopicOptions('', 'posting')}</select></label><label class="kb-field"><span>Title</span><input name="title" required></label><label class="kb-field"><span>Starting note</span><textarea name="body"></textarea></label><div class="kb-dialog-actions"><button type="button" class="secondary-btn" data-kb-close="kbContentDialog">Cancel</button><button class="primary-btn" type="submit">Create idea</button></div></form></dialog>`;
}

function kbTabBody(){
  if(kbTab==='sources')return `<section class="kb-tab-panel"><div class="kb-section-head"><div><h2>Information sources</h2><p>The source database: current product site, documentation, Telegram, YouTube, team input and external research watchlists.</p></div><button class="primary-btn" data-kb-add-source>Add source</button></div><div class="kb-source-grid">${kbSourceBlocks()}</div></section>`;
  if(kbTab==='structure')return `<section class="kb-tab-panel"><div class="kb-section-head"><div><h2>Information structure</h2><p>Product knowledge organized from the actual IntraWeb 17 product page and documentation: product, authoring, IWML, runtime, deployment, architecture, comparisons and lifecycle.</p></div><button class="primary-btn" data-kb-add-item>Add knowledge / idea</button></div><div class="kb-filterbar"><input data-kb-search value="${ke(kbSearch)}" placeholder="Search knowledge"><select data-kb-group-filter><option value="all">All source blocks</option>${KB.sourceGroups.map(g=>`<option value="${ke(g.id)}" ${kbGroup===g.id?'selected':''}>${ke(g.name)}</option>`).join('')}</select><select data-kb-topic-filter><option value="all">All information topics</option>${kbTopicOptions(kbTopic==='all'?'':kbTopic,'information')}</select></div><div class="kb-workspace"><section class="kb-panel"><div class="kb-panel-head"><div><h2>Knowledge tree</h2><p>Resulting product structure, independent from the editorial posting taxonomy.</p></div><span class="pill">${kbTopics('information').length} topics</span></div><div class="kb-tree">${kbTopicTree()}</div></section><section class="kb-panel"><div class="kb-panel-head"><div><h2>Knowledge records</h2><p>${kbItems().length} items match the current filters.</p></div></div><div class="kb-items">${kbKnowledgeRows()}</div></section></div></section>`;
  return `<section class="kb-tab-panel"><div class="kb-section-head"><div><h2>Posting topics</h2><p>Possible editorial directions derived from the verified product structure, grouped for posts, shorts, reels and long videos.</p></div></div><div class="kb-panel"><div class="kb-panel-head"><div><h2>Editorial topic tree</h2><p>Choose a topic and create the next content record directly from it.</p></div><span class="pill accent">${kbTopics('posting').length} possible topics · ${KB.content.length} records</span></div><div class="kb-tree kb-posting-tree">${kbPostingTree()}</div></div></section>`;
}

function kbRender(){
  if(!KB||kbRoute()!=='knowledge')return;const view=kq('#view');if(!view)return;
  const info=kbTopics('information'),leaves=info.filter(t=>!kbChildren(t.id).length),covered=leaves.filter(t=>kbItemCountForTopic(t.id)>0).length;
  kq('#pageCrumb').textContent='Knowledge Base';
  view.innerHTML=`<div class="kb-page"><div class="kb-compact-head"><div><h1>Knowledge Base</h1><span>${KB.sources.filter(s=>s.enabled!==false).length} sources · ${KB.knowledgeItems.length} knowledge items · ${covered}/${leaves.length} information leaves covered</span></div></div><nav class="kb-tabs" aria-label="Knowledge Base sections"><button data-kb-tab="sources" class="${kbTab==='sources'?'active':''}">Information sources</button><button data-kb-tab="structure" class="${kbTab==='structure'?'active':''}">Information structure</button><button data-kb-tab="posting" class="${kbTab==='posting'?'active':''}">Posting topics</button></nav>${kbTabBody()}${kbDialogs()}</div>`;
  view.dataset.routeOwner='knowledge-base';view.dataset.route='knowledge';kbBind();
}

function kbRecommendedPostingTopic(item){
  for(const id of item?.topicIds||[]){const target=kbTopicById(id)?.postingTopicId;if(target&&kbTopicById(target)?.treeType==='posting')return target}
  return kbTopics('posting').find(t=>!t.parentTopicId)?.id||'';
}
function kbOpenContent({itemId='',topicId=''}){
  ...
  f.reset();f.elements.knowledgeItemId.value=item?.id||'';f.elements.topicId.value=topicId||kbRecommendedPostingTopic(item);f.elements.title.value=item?.kind==='idea'?item.title.replace(/^Content idea:\s*/i,''):item?.title||'';f.elements.body.value=item?.summary||'';
  kq('[data-kb-lineage]').textContent=item?`Lineage: ${kbSource(item.sourceId)?.name||'source'} → ${(item.topicIds||[]).map(id=>kbTopicById(id)?.name).filter(Boolean).join(' / ')||'information'} → ${kbTopicById(f.elements.topicId.value)?.name||'posting topic'} → content.`:'This content idea will be linked to the selected posting topic.';
  d.showModal();
}
